...
class RP61_tipo_intervento(Variable):
    value_type = Enum
    ...
    label = "RP61 Col.1 - Indicare il codice del tipo di intervento finalizzato al risparmio energetico"
...
class RP61_anno_sostenimento_spese_risparmio_energetico(Variable):
    value_type = int
    entity = Persona
    definition_period = YEAR
    entity = Persona
    label = "RP61 Col.2 - Indicare il anno sostenimento spesa intervento finalizzaot al risparmio energetico"
    reference = "http://www.agenziaentrate.gov.it/wps/file/Nsilib/Nsi/Schede/Dichiarazioni/Redditi+Persone+fisiche+2018/Modello+e+istruzioni+Redditi+PF2018/Istruzioni+Redditi+Pf+-+Fascicolo+1+2018/PF1_istruzioni_2018_Ret.pdf#page=78"  # Always use the most official source


class RP61_periodo_2013(Variable):
    value_type = Enum
    ...
    entity = Persona
    definition_period = YEAR
    label = "RP61 Col.3 - Compilare questa colonna solo se le spese sono state sostenute nel 2013"
    reference = "http://www.agenziaentrate.gov.it/wps/file/Nsilib/Nsi/Schede/Dichiarazioni/Redditi+Persone+fisiche+2018/Modello+e+istruzioni+Redditi+PF2018/Istruzioni+Redditi+Pf+-+Fascicolo+1+2018/PF1_istruzioni_2018_Ret.pdf#page=78"  # Always use the most official source


class RP61_casi_particolari(Variable):
    value_type = Enum
...
class RP61_Range_Spesa_Date(Variable):
    value_type = Enum
...
class RP61_limite_spesa_in_base_a_codice_da_1_a_4(Variable):
    value_type = float
    entity = Persona
    definition_period = YEAR
...
class RP61_spesa_totale(Variable):
    value_type = float
    entity = Persona
    definition_period = YEAR
    label = "RP61 Col.8 - indicare l’ammontare della spesa sostenuta entro i limiti sottodescritti in relazione alla tipologia dell’intervento che, ad eccezione delle spese indicate con il codice 7 "
    reference = "http://www.agenziaentrate.gov.it/wps/file/Nsilib/Nsi/Schede/Dichiarazioni/Redditi+Persone+fisiche+2018/Modello+e+istruzioni+Redditi+PF2018/Istruzioni+Redditi+Pf+-+Fascicolo+1+2018/PF1_istruzioni_2018_Ret.pdf#page=78"  # Always use the most official source
...
class RP61_importo_rata(Variable):
    ...
    def formula(person,period,parameters):
        ...
        RP61_limite_spesa_in_base_a_codice_da_1_a_4 = person('RP61_limite_spesa_in_base_a_codice_da_1_a_4',period)
        RP61_limite_spesa_in_base_a_codice_5_6 = person('RP61_limite_spesa_in_base_a_codice_5_6',period)
        # il rigo ha un solo nella colonna 1 quindi prendo il limite diverso da 0
        limite_da_utilizzare = select([not_(RP61_limite_spesa_in_base_a_codice_da_1_a_4 == 0),
                                        not_(RP61_limite_spesa_in_base_a_codice_5_6 == 0)],
                                [RP61_limite_spesa_in_base_a_codice_da_1_a_4,RP61_limite_spesa_in_base_a_codice_5_6])
        # controllo valore spesa totale
        spesa_totale = where (person('RP61_spesa_totale',period)<limite_da_utilizzare, person('RP61_spesa_totale',period),limite_da_utilizzare)
        rideterminazione_rata_compilato = not_(person('RP61_periodo_2008_rideterminazione_rate',period) == 0)
        rate_utilizzate = (8 - person('RP61_numero_rata',period)) * (spesa_totale/person('RP61_periodo_2008_rideterminazione_rate',period))
